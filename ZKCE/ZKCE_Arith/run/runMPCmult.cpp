...
typedef struct {
  ...
  ZZ* Rcom[5] = {new ZZ[N], new ZZ[N], new ZZ[N], new ZZ[N], new ZZ[N]};
...
void commit(ZZ* C[2], ZZ* mx, ZZ* r[5], ZZ* A1[5], ZZ* A2[5], Ring ring, long np,ZZ QQ) {
  ZZ* tmp = new ZZ[N];
  ZZ* tmp1 = new ZZ[N];
...
void commitVec(ZZ* C[4], ZZ* mx[3], ZZ* r[5], ZZ* A[7], Ring ring, long np,ZZ QQ) {
  ZZ* tmp = new ZZ[N];
  ZZ* tmp1 = new ZZ[N];
  ...
  ring.add(C[0], tmp, tmp1, QQ);
  ring.mult(tmp1, A[2], r[3], np, QQ);
  ring.addAndEqual(C[0], tmp1, QQ);
  ring.mult(tmp1, A[3], r[4], np, QQ);
  ring.addAndEqual(C[0], tmp1, QQ);
  ring.addAndEqual(C[0], r[0], QQ);

  /// ->> C2
  ring.mult(tmp, A[4], r[4], np, QQ);
  ring.addAndEqual(tmp, r[1], QQ);
  ring.add(C[1], tmp, mx[0], QQ);
  
  /// ->> C3
  ring.mult(tmp, A[5], r[4], np, QQ);
  ring.addAndEqual(tmp, r[2], QQ);
  ring.add(C[2], tmp, mx[1], QQ);

  /// ->> C4
  ring.mult(tmp, A[6], r[4], np, QQ);
  ring.addAndEqual(tmp, r[3], QQ);
  ring.add(C[3], tmp, mx[2], QQ);
}


void genMatricesVec( ZZ* r[5], ZZ* A[7], Ring ring, long np, ZZ QQ) {
  for (int i; i<7; i++){
    ring.sampleUniform2(A[i], logQQ); // Need to check the norm bound
  }
  for (int i; i<5; i++){
    ring.sampleUniform2(r[i], logQQ); // Need to check the norm bound
  }
}
...
string comToString(ZZ** r, long n) {
  return StringUtils::toString(r[0], n) + StringUtils::toString(r[1], n) + StringUtils::toString(r[2], n) + StringUtils::toString(r[3], n) + StringUtils::toString(r[4], n);
}
...
void noisedView(Ring ring, ZZ* vx, ZZ* m0, ZZ* m1, ZZ** mx, ZZ** r_com) {  
  ...
  ring.sampleUniform2(r_com[3], 4);
  ring.sampleUniform2(r_com[4], 4);
  mx[0] = vx;
  mx[1] = m0;
  mx[2] = m1;
}
...
void noisedView2(Ring ring, Player player) {  
  ...
  ring.sampleUniform2(player.Rcom[3], 4);
  ring.sampleUniform2(player.Rcom[4], 4);
...
void splitNoises(Ring ring, ZZ qQ0, ZZ* vx, ZZ* m0, ZZ* m1, ZZ** mx, ZZ** r_com, ZZ** C_com, ZZ* vx1, ZZ* m01, ZZ* m11, ZZ** mx1, ZZ** r_com1, ZZ** C_com1, ZZ* vx2, ZZ* m02, ZZ* m12, ZZ** mx2, ZZ** r_com2, ZZ** C_com2, ZZ* vx3, ZZ* m03, ZZ* m13, ZZ** mx3, ZZ** r_com3, ZZ** C_com3) {
  ...
  ring.add(tmp, r_com1[3], r_com2[3], qQ0);
  ring.sub(r_com3[3], r_com[3], tmp, qQ0);
  ring.add(tmp, r_com1[4], r_com2[4], qQ0);
  ring.sub(r_com3[4], r_com[4], tmp, qQ0);
  mx3[0] = vx3;
  mx3[1] = m03;
  mx3[2] = m13;
  //delete[] tmp;
...
void splitNoises2(Ring ring, ZZ qQ0, Player player0, Player player1, Player player2, Player player3) {
  ...
  ring.add(tmp, player1.Rcom[3], player2.Rcom[3], qQ0);
  ring.sub(player3.Rcom[3], player0.Rcom[3], tmp, qQ0);
  ring.add(tmp, player1.Rcom[4], player2.Rcom[4], qQ0);
  ring.sub(player3.Rcom[4], player0.Rcom[4], tmp, qQ0);
...
int main() {
  // Parameters //
  long logq = 45; ///< Ciphertext modulus (this value should be <= logQ in "scr/Params.h")
  long logp = 45; ///< Scaling Factor (larger logp will give you more accurate value)
...
timeutils.start(ZKGEN, 0);
  ...
  Player player0;
  ZZ* A_com[7] = {new ZZ[N], new ZZ[N], new ZZ[N], new ZZ[N], new ZZ[N], new ZZ[N], new ZZ[N]};
...
// Generate noises
  ...
  //encryptInput(scheme, ring, ring2, player0.plain, player0.cipher, logp, logq, n, np, key, player0.mvec, A_com, player0.MX, player0.Rcom, player0.Ccom, player0.VX, player0.M0, player0.M1, QQ);
...
// Create views for noise 
  ...
  //splitNoises2(ring, qQ0, player0, player1, player2, player3); 
...
timeutils.start(ZKVER, 0);
  ...
  ZZ* m0_e_v = new ZZ[N];
  ZZ* m1_e_v = new ZZ[N];
  ZZ* r_com_e_v[5] = {new ZZ[N], new ZZ[N], new ZZ[N], new ZZ[N], new ZZ[N]};
  ...
  ZZ* m0_e1_v = new ZZ[N];
  ZZ* m1_e1_v = new ZZ[N];
  ZZ* r_com_e1_v[5] = {new ZZ[N], new ZZ[N], new ZZ[N], new ZZ[N], new ZZ[N]};
...
// Seed is artificially retrived from k1_v resp. k2_v. We reuse the memspace allocated prior.
    ...
    r_com_e1_v[3] = zs.rv_[3];
    r_com_e1_v[4] = zs.rv_[4];
    mx_e1_v[0] = vx_e1_v;
    mx_e1_v[1] = m0_e1_v;
    mx_e1_v[2] = m1_e1_v;
  ...
  // Seed is artificially retrived from k1_v resp. k2_v. We reuse the memspace allocated prior.
    ...
    r_com_e_v[3] = zs.rv_[3];
    r_com_e_v[4] = zs.rv_[4];
    mx_e_v[0] = vx_e_v;
    mx_e_v[1] = m0_e_v;
    mx_e_v[2] = m1_e_v;
