...
void Ring::addGaussAndEqualAndSave(ZZ* res, ZZ* mi, const ZZ& q) {
	static double Pi = 4.0 * atan(1.0);
	static long bignum = 0xfffffff;
	...
	for (long i = 0; i < N; i+=2) {
		...
		mi[i] = (long) floor(rr * cos(theta) + 0.5);
		mi[i + 1] = (long) floor(rr * sin(theta) + 0.5);

		AddMod(res[i], res[i], mi[i], q);
		AddMod(res[i + 1], res[i + 1], mi[i + 1], q);
	}
}

void Ring::sampleGauss(ZZ* mi) {
	static double Pi = 4.0 * atan(1.0);
	static long bignum = 0xfffffff;
...
void Ring::decomposeNoise(ZZ* m, ZZ* m1, ZZ* m2, ZZ* m3, const ZZ& q) {
	ZZ* tmp = new ZZ[N];
	sampleZO(m1); 
	sampleZO(m2);
	add(tmp, m1, m2, q);
...
void Ring::decomposeNoise(ZZ* m, ZZ* m1, ZZ* m2, ZZ* m3, const ZZ& q, const unsigned char data1, const unsigned char data2) {
	ZZ* tmp = new ZZ[N];
  	SetSeed(&data2, 4);
	sampleZO(m2);
  	SetSeed(&data1, 4);  
...
void Ring::decomposeNoise(ZZ* m, ZZ* m1, ZZ* m2, ZZ* m3, const ZZ& q, int k1, int k2, int offset) {
	ZZ* tmp = new ZZ[N];
	const unsigned char data1 = (unsigned char)k1+offset;
  	SetSeed(&data1, 4);  
	sampleZO(m1); 
	const unsigned char data2 = (unsigned char)k2+offset;
  	SetSeed(&data2, 4);
	sampleZO(m2);
	add(tmp, m1, m2, q);
	sub(m3, m, tmp, q);
...
void Ring::compare(ZZ* m1, ZZ* m2) {
	ZZ tmp = (ZZ)0;
	ZZ max_ = (ZZ)0;
	ZZ min_ = (ZZ)0;
	int count = 0;
	long bit = 0; // Does the bit shift 
   	rightShiftAndEqual(m1, bit);
	leftShiftAndEqual(m1, bit,QQ);
  	rightShiftAndEqual(m2, bit);
	leftShiftAndEqual(m2, bit,QQ);

	for (long i = 0; i < N; ++i) {
		if (m1[i] != m2[i]) {

			std::cout<<"[Error] at index "<< i << ", diff : "<< m1[i] - m2[i] << std::endl;
			count += 1;
		}
		tmp += (m1[i] - m2[i]);
		if((m1[i] - m2[i])<min_)
...
void Ring::compareNoisy(ZZ* m1, ZZ* m2) {
	ZZ tmp = (ZZ)0;
	ZZ max_ = (ZZ)0;
	ZZ min_ = (ZZ)0;
	int count = 0;
	...
	int bound = 1000000;  // Added to account for the discretization that occurs in the NTT // TO BE REMOVED
	for (long i = 0; i < N; ++i) {
		m1[i] = (m1[i]/bound);
		m2[i] = (m2[i]/bound);
	}
