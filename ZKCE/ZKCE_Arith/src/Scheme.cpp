...
void Scheme::encrypt(Ciphertext& cipher, Plaintext plain, complex<double>* mvec, long n, long logp, long logq, long np, ZZ* vx, ZZ* m0, ZZ* m1) {
  encode(plain, mvec, n, logp, logq);
  cipher.logp = plain.logp;
  cipher.logq = plain.logq;
  cipher.n = plain.n;
  ZZ qQ = ring.qpows[plain.logq + logQ];
  ring.sampleZO(vx); // VX  __________________________
  Key* key = keyMap.at(ENCRYPTION);
  ring.multNTT(cipher.ax, vx, key->rax, np, qQ);
  ring.addGaussAndEqualAndSave(cipher.ax, m1, qQ); // m1 _______
  ring.multNTT(cipher.bx, vx, key->rbx, np, qQ);
  ring.addGaussAndEqualAndSave(cipher.bx, m0, qQ); // m0 ________
  ring.addAndEqual(cipher.bx, plain.mx, qQ);
  ring.rightShiftAndEqual(cipher.ax, logQ);
  ring.rightShiftAndEqual(cipher.bx, logQ);
}
...
void Scheme::multByConstVec(Ciphertext& res, Ciphertext& cipher, double* cnstVec, long logp) {
	...
	ZZ cnstZZ;
	for (long i = 0; i < N; ++i) {
		cnstZZ = EvaluatorUtils::scaleUpToZZ(cnstVec[i%(N/2)], logp);
		cout<<"cnstZZ["<<i<<"] "<<cnstVec[i]<<endl;
		MulMod(res.ax[i], cipher.ax[i], cnstZZ, q);
		MulMod(res.bx[i], cipher.bx[i], cnstZZ, q);
	}
	cipher.logp += logp;
}

void Scheme::multByConstVec(Ciphertext& res, Ciphertext& cipher, Plaintext& cnstVec, long logp) {
	// Maison
	ZZ q = ring.qpows[cipher.logq];
	cout<<"hello mate"<<endl;
	ZZ cnstZZ;
	for (long i = 0; i < N; ++i) {
		MulMod(res.ax[i], cipher.ax[i], cnstVec.mx[i], q);
		MulMod(res.bx[i], cipher.bx[i], cnstVec.mx[i], q);
	}
	cipher.logp += logp;
}

void Scheme::multByConstVecSlot(Ciphertext& res, Ciphertext& cipher, long logp) {
	// Maison
	ZZ q = ring.qpows[cipher.logq];
	ZZ cnstZZ;
	for (long i = 0; i < N; ++i) {
		res.ax[i] = cipher.ax[i];
		res.bx[i] = cipher.bx[i];
	}
	cnstZZ = EvaluatorUtils::scaleUpToZZ((double)0, logp);
	MulMod(res.ax[0], cipher.ax[0], cnstZZ, q);
	MulMod(res.bx[0], cipher.bx[0], cnstZZ, q);
	//MulMod(res.ax[N-1], cipher.ax[N-1], cnstZZ, q);
	//MulMod(res.bx[N-1], cipher.bx[N-1], cnstZZ, q);
	cipher.logp += logp;
}
